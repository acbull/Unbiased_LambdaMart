...
void GBDT::Boosting() { /// 计算全部样本的gradients和hessians
  if (objective_function_ == nullptr) {
...
void GBDT::Train(int snapshot_freq, const std::string& model_output_path) { /// GBDT训练
  bool is_finished = false;
  auto start_time = std::chrono::steady_clock::now();
  for (int iter = 0; iter < gbdt_config_->num_iterations && !is_finished; ++iter) { /// 迭代次数
    is_finished = TrainOneIter(nullptr, nullptr); /// 按gradients拟合一颗新树
    if (!is_finished) { /// 成功TrainOneIter
      is_finished = EvalAndCheckEarlyStopping(); /// 评估当前iter_结果，是否earlystop，对训练过程无影响
    }
...
bool GBDT::TrainOneIter(const score_t* gradients, const score_t* hessians) {
  ...
  for (int cur_tree_id = 0; cur_tree_id < num_tree_per_iteration_; ++cur_tree_id) {
    ...
    std::unique_ptr<Tree> new_tree(new Tree(2)); /// 当前树
    if (class_need_train_[cur_tree_id]) {
      ...
      new_tree.reset(tree_learner_->Train(grad, hess, is_constant_hessian_, forced_splits_json_)); /// 用一颗树拟合grad，并赋给new_tree
    }
    ...
    if (new_tree->num_leaves() > 1) { /// 新树成功建立
    ...
    // add model
    models_.push_back(std::move(new_tree)); /// 增加一颗新树
  }

  if (!should_continue) { /// stop training
...
bool GBDT::EvalAndCheckEarlyStopping() {
  ...
  auto best_msg = OutputMetric(iter_); /// 评测iter_
...
std::vector<double> GBDT::EvalOneMetric(const Metric* metric, const double* score) const { /// 按metric评测一次
...
std::string GBDT::OutputMetric(int iter) { ///
  ...
  if (need_output) {
    for (auto& sub_metric : training_metrics_) {
      ...
      auto scores = EvalOneMetric(sub_metric, train_score_updater_->score()); /// 得到整体ndcg得分
