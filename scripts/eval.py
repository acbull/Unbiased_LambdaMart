...
os.system('./trec_eval -c -m ndcg_cut.1,3,5,10 ' + data_dir + 'test/test.qrels ./trec_out.txt > res.out')
print(''.join(open('./res.out').readlines()))
os.system('./trec_eval -c ' + data_dir + 'test/test.qrels ./trec_out.txt > res.out')
print(''.join(open('./res.out').readlines()))
